...
Original file is located at
    https://colab.research.google.com/drive/1SKlfGETl6E_9KddfWrmhoJgCy-lvqkMB
"""
...
K = 14
DFT_timecompl = np.zeros(K)
FFT_timecompl = np.zeros(K)
...
for i in range(K):
    ...
    t4 = time.time()
    DFT_timecompl[i] = t2-t1
    FFT_timecompl[i] = t3-t2
    FFTinBuilt_comp_time[i] = t4-t3

ax = 2**np.arange(K)
plt.plot(ax,DFT_timecompl,label='DFT')
plt.plot(ax,FFT_timecompl,label='FFT')
plt.plot(ax,FFTinBuilt_comp_time,label='Inbuilt FFT')
plt.title('Computation Times analysis')
plt.xlabel('n')
